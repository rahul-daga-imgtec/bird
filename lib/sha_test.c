...
#define define_test_sha_fn(x) 						\
static int								\
test_sha##x(void *out_, const void *in_, const void *expected_out_)	\
{									\
  char *out = out_;							\
  const char *in = in_;							\
  const char *expected_out = expected_out_;				\
									\
  struct sha##x##_context ctx;						\
  sha##x##_init(&ctx);							\
  sha##x##_update(&ctx, in, strlen(in));				\
  byte *out_bin = sha##x##_final(&ctx);					\
									\
  bt_bytes_to_hex(out, out_bin, SHA##x##_SIZE);				\
									\
  return strncmp(out, expected_out, SHA##x##_HEX_SIZE) == 0 ? BT_SUCCESS : BT_FAILURE; \
}

define_test_sha_fn(1)
define_test_sha_fn(224)
define_test_sha_fn(256)
define_test_sha_fn(384)
define_test_sha_fn(512)
...
/* the data structure in md5_test.c */
...
/* the same function in md5_test.c */
static void
...
#define define_test_sha_hmac_fn(x)						\
static int									\
test_sha##x##_hmac(void *out_, const void *in_, const void *expected_out_)	\
{										\
  char *out = out_;								\
  const struct hmac_data_in *in = in_;						\
  const char *expected_out = expected_out_;					\
										\
  struct sha##x##_hmac_context ctx;						\
  sha##x##_hmac_init(&ctx, in->key, in->key_len);				\
  sha##x##_hmac_update(&ctx, in->data, in->data_len);				\
  byte *out_bin = sha##x##_hmac_final(&ctx);					\
  bt_bytes_to_hex(out, out_bin, SHA##x##_SIZE);					\
										\
  return strncmp(out, expected_out, SHA##x##_HEX_SIZE) == 0 ? BT_SUCCESS : BT_FAILURE; \
}

define_test_sha_hmac_fn(1)
define_test_sha_hmac_fn(224)
define_test_sha_hmac_fn(256)
define_test_sha_hmac_fn(384)
define_test_sha_hmac_fn(512)
