...
nexthop_merge_rta(struct nexthop *nhs, rta *a, int max)
{
  return nexthop_merge(nhs, &(a->nh), 1, 0, max, rte_update_pool);
}
  ...
  *rt_free = NULL;
  ...
  for (rt0 = best0->next; rt0; rt0 = rt0->next)
  {
    ...
    if (rte_is_reachable(rt))
      nhs = nexthop_merge_rta(nhs, rt->attrs, c->merge_limit);
  ...
  if (nhs)
  {
    nhs = nexthop_merge_rta(nhs, best->attrs, c->merge_limit);
